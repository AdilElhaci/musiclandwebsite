...
public class PagintionControllers {    
       // page : 1 baslar
    ...
    @Inject
     UyelikYapanlarControllers user;
    ...
    public int sayfaNumarasi() {
         ...
         */ 
        int x = (this.page - 1) / getSayfa_Adet();// peryodu belirliyor
    ...
    public void next() {
        if (page >= this.getPage_count()) 
            this.page = 1;// son sayfa yada bos bir sayfaya geldiysem next basilinca ilk sayfaya gitsin
         else 
            page ++;
    }
    public void prev() {
    ...
    public int getPage_count() {
        int count = 0;
        if(getDao() instanceof AlbumDao){
            count = ((AlbumDao)getDao()).findCount(getSearchTerimi());
        }
        else if(getDao() instanceof CategoryDao){
            count = ((CategoryDao)getDao()).findCount(this.getSearchTerimi());
        }
        else if(getDao() instanceof CeviriDillerDao){            
            count = ((CeviriDillerDao)getDao()).findCount(getSearchTerimi());
        }
        else if(getDao() instanceof DilDao){
            count = ((DilDao)getDao()).findCount(getSearchTerimi());
        }
        else if(getDao() instanceof ExtraDao){
            count = ((ExtraDao)getDao()).findCount(getSearchTerimi());
        }
        else if(getDao() instanceof KullaniciDao){
            count = ((KullaniciDao)getDao()).findCount(getSearchTerimi(),getYetki_saviye());
        }
        else if(getDao() instanceof KullaniciSarkiListesiDao){            
           count = ((KullaniciSarkiListesiDao)getDao()).findCount(this.getUser().getUpdateToKullanici().getKullanici_id(),getSearchTerimi());                    
        }
        else if (getDao() instanceof SarkiDocumentDao) {
            count = ((SarkiDocumentDao) getDao()).findCount(getSearchTerimi(),getId());            
        } 
        else if(getDao() instanceof SarkiDao){
            count = ((SarkiDao) getDao()).findCount(getSearchTerimi(),getId());  
        }
        else if(getDao() instanceof SarkiciDao){
            count = ((SarkiciDao) getDao()).findCount(getSearchTerimi());
        }
        else if(getDao() instanceof UlkeDao) {
            count = ((UlkeDao) getDao()).findCount(getSearchTerimi());
        }
    ...
    public UyelikYapanlarControllers getUser() {
        return user;
    }
